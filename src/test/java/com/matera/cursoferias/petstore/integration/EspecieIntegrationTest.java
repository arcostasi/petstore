	...
	public void criaEspecieSemDescricaoErro() {
		...
		given()
			...
			.then()
				...
				.body("errors[0]", Matchers.equalTo("descricao: Descrição é de preenchimento obrigatório."));
	}
	...
	public void criaEspecieComDescricaoExcedendoLimiteErro() {
		...
		given()
			...
			.then()
				.statusCode(HttpStatus.BAD_REQUEST.value())
				.body("errors.size()", Matchers.is(1))
				.body("errors[0]", Matchers.equalTo("descricao: Descrição não pode ultrapassar 100 caracteres."));
	}
	...
	public void apagaEspecieComPetErro() {
		...
		given()
	      ...
	      .then()
		  	.statusCode(HttpStatus.BAD_REQUEST.value())
		  	.body("errors.size()", Matchers.is(1))
			.body("errors[0]", Matchers.equalTo(String.format("Espécie %d não pode ser excluída pois possui Pets!", especieResponse.getId())));
	}
